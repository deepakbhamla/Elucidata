...
class FileUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
      ...
      if file_serializer.is_valid():
          ...
          GDRAT_abs_path = os.path.join(MEDIA_ROOT + '/result/task2.xlsx')
          ...
          GDRAT_abs_path = os.path.join(MEDIA_ROOT + '/result/task3.xlsx')
...
def LPCDownloadView(request):
    file_obj = os.path.join(MEDIA_ROOT + '/result/LPC.xlsx')
...
def PCDownloadView(request):
    file_obj = os.path.join(MEDIA_ROOT + '/result/PC.xlsx')
    document = open(file_obj, 'rb')   
    response = HttpResponse(FileWrapper(document),content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def PlasmalogenDownloadView(request):
    file_obj = os.path.join(MEDIA_ROOT + '/result/PLASMA.xlsx')
    document = open(file_obj, 'rb')   
    response = HttpResponse(FileWrapper(document),content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def RoundDownloadView(request):
    file_obj = os.path.join(MEDIA_ROOT + '/result/task2.xlsx')
    document = open(file_obj, 'rb')   
    response = HttpResponse(FileWrapper(document),content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def MeanDownloadView(request):
    file_obj = os.path.join(MEDIA_ROOT + '/result/task3.xlsx')
    document = open(file_obj, 'rb')   
    response = HttpResponse(FileWrapper(document),content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
